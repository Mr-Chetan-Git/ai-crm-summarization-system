...
def run_toy_summarization_chetan(show_samples=True):

    print("STEP 1 : Loading device")

    device = config.DEVICE

    print(f"Using device : {device}")

    print("STEP 2 : Building tokenizer, dataset and Transformer model")
    ...
    print("Transformer encoder-decoder model created")
    ...
    loader = DataLoader(
        dataset,
        batch_size=config.SUMMARIZATION_BATCH_SIZE,
        shuffle=True
    )

    print(f"Batch size : {config.SUMMARIZATION_BATCH_SIZE}")
    ...
    optimizer = torch.optim.Adam(
        model.parameters(),
        lr=config.SUMMARIZATION_LR
    )

    print(f"Learning rate : {config.SUMMARIZATION_LR}")
    
    print("STEP 6 : Starting Transformer training")
    ...
    for epoch in range(1, config.SUMMARIZATION_EPOCHS + 1):
        ...
        for batch in loader:
            ...
            print("Input tensors moved to device")


            print("Creating source mask")

            src_mask = TransformerModel.create_src_mask(
                src_ids,
                tokenizer.pad_id
            )
            ...
            tgt_mask = TransformerModel.create_tgt_mask(
                tgt_in_ids,
                tokenizer.pad_id
            )
            ...
            print("Updating Transformer weights")
        ...
        print(
            f"\nEpoch {epoch:02d}/{config.SUMMARIZATION_EPOCHS}"
        )

        print(
            f"Average Loss : {running_loss / len(loader):.4f}"
        )


    print("STEP 7 : Transformer training completed")
    

    if show_samples:


        print("STEP 8 : Testing sample summaries")
        

        sample_comments = [ 


            "20 May - cannot receive incoming calls 19may-again asked to send syllabus 19may-no answer 18may-no answer 16may-no answer 15may-syllabus sent again  14may-no answer 13may-he asked to send syllabus again 12may-no answer 11may-asked to send syllabus again coz he didnt go through it 9may-no response 8may-asked to send syllabus again, hes busy in meetings 5may-hasnt gone through syllabus,yt. call on friday 4may-hasnt gone through syllabus,yt 30-Apr Naveen had a syllabus walkthrough"
        ]
        ...
        for text in sample_comments:
            ...
            print("Generating summary using Transformer decoder")
    ...
    print("STEP 9 : Returning model and tokenizer")
...
def main():
    ...
    if args.task == "summarization":
        model, tokenizer = run_toy_summarization_chetan(show_samples=args.predict_text is None)
...
def generate_summary_text(text):
    ...
    tokens = tokens[:config.MAX_SRC_LEN]

    src = torch.tensor([tokens]).to(config.DEVICE)

    tgt_input = torch.tensor(
        [[tokenizer.token_to_id["<SOS>"]]]
    ).to(config.DEVICE)

    for _ in range(config.MAX_TGT_LEN):
    ...
    summary_tokens = tgt_input[0].tolist()
